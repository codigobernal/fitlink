...
function calculateEffort(
  ...
  const actualCalories = calculateCalories(
    age,
    weight,
    heightMeters,
    sex,
  ...
  let effortPercent =
    maxCaloriesTheoric > 0 ? (actualCalories / maxCaloriesTheoric) * 100 : 0;
...
export default function Home() {
  ...
  const [bmiForm, setBmiForm] = useState<BmiForm>({
    weight: "",
    height: "",
    age: "",
    sex: "",
  ...
  const bmiSex =
    bmiForm.sex.trim().toUpperCase() === "M"
      ? "M"
      : bmiForm.sex.trim().toUpperCase() === "F"
        ? "F"
        : "";

  const computedBmi =
    heightMeters > 0 ? parsedWeight / (heightMeters * heightMeters) : 0;
  ...
  const currentRouteSpeed = useMemo(() => {
    return elapsed > 0 && routeDistanceKm > 0
      ? routeDistanceKm / (elapsed / 3600)
      : 0;
  }, [elapsed, routeDistanceKm]);

  // Pasos y Calorías se calculan en tiempo real para la sesión actual
  const steps = useMemo(
    () => calculateSteps(routeDistanceKm, heightMeters, bmiSex),
  ...
  const calories = useMemo(
    ...
    [parsedAge, parsedWeight, heightMeters, bmiSex, routeDistanceKm],
  );
  const computedEffort = useMemo(
    ...
    [parsedAge, parsedWeight, heightMeters, bmiSex, routeDistanceKm, elapsed],
  );
  ...
  const displayData = tracking
    ...
    : {
        ...
        lastMs: lastSavedSession?.timestamp
          ? toMillis(lastSavedSession.timestamp)
          : lastMs,
      };
  ...
  const dailySummary = useMemo(() => {
    ...
    const totalDistance = sessionsDia.reduce(
      (sum, s) => sum + s.distanciaFinal,
      0,
    );
    const totalTime = sessionsDia.reduce((sum, s) => sum + s.tiempoFinal, 0);
    const totalCalories = sessionsDia.reduce(
      (sum, s) => sum + s.caloriasFinales,
      0,
    );
  ...
  const handleSaveBmi = async () => {
    ...
    if (!currentUser) {
      Alert.alert(
        "Sesión expirada",
        "Inicia sesión nuevamente para guardar tus datos.",
      );
      return;
    }
    const payload = {
  ...
  const saveSessionData = useCallback(
    async (
      ...
      const sessionData = {
        ...
        routePoints: route.length > 1000 ? null : JSON.stringify(route), // <--- Asegurar que se guarde como JSON
      };
  ...
  const startTracking = useCallback(async () => {
    try {
      ...
      if (status !== Location.PermissionStatus.GRANTED) {
        ...
        setModalMessage(
          "Necesitamos acceso a tu ubicación para registrar la ruta.",
        );
        setModalVisible(true);
        return;
      }
  ...
  useEffect(() => {
    ...
    const unsub = onValue(q, (snap) => {
      ...
      const arr: Lectura[] = Object.keys(data).map((key) => ({
        id: key,
        ...data[key],
      }));
  ...
  useEffect(() => {
    ...
    const off = onAuthStateChanged(auth, (user) => {
      ...
      const sessionsRef = query(
        ref(db, `users/${user.uid}/sessions`),
        orderByChild("timestamp"),
        limitToLast(50),
      );
    ...
    const off = onAuthStateChanged(auth, (user) => {
      ...
      detachMetrics = onValue(metricsRef, (snapshot) => {
        ...
        const heightVal = parseFloat(
          String(data.height ?? "").replace(",", "."),
        );
        const normalizedHeight = Number.isFinite(heightVal)
          ? heightVal > 5
            ? heightVal / 100
            : heightVal
          : NaN;
  ...
  const statItems: StatItem[] = [
    ...
    },
    {
      ...
      v:
        dailySummary.totalDistance > 0
          ? `${Number(dailySummary.totalDistance).toFixed(2)} km`
          : "0 km",
      vc: "#C084FC",
      iconName: "walk",
    },
    {
      c: "#50d8fdff",
      t: "Pasos",
      v: dailySummary.totalSteps > 0 ? `${dailySummary.totalSteps}` : "0",
      vc: "#50d8fdff",
      iconName: "footsteps",
    },
    {
      ...
      t: "Rapidez",
      v:
        dailySummary.avgSpeed > 0
          ? `${Number(dailySummary.avgSpeed).toFixed(2)} km/h`
          : "0 km/h",
      vc: "#42c700ff",
    ...
    },
    {
      ...
      v:
        dailySummary.totalTime > 0
          ? `${formatTime(dailySummary.totalTime)}`
          : "0:00",
      vc: "#bf0101ff",
      iconName: "time",
    },
    ...
    <SafeAreaView style={styles.safe}>
      ...
      >
        <View style={styles.headerRow}>
          ...
          <Pressable onPress={() => router.push("/profile")}>
            <View
              style={[
                styles.headerIcon,
                { backgroundColor: profileIcon.color || "#2B2B2E" },
              ]}
            >
              <Ionicons
                name={profileIcon.name || "person"}
                size={18}
                color="#111"
              />
            </View>
        ...
        <View style={[styles.cardSmall, { padding: 0, overflow: "hidden" }]}>
          <Image
            source={require("../../assets/elements/prom4.gif")}
          ...
          <View style={styles.bmiRow}>
            ...
            <View style={styles.bmiItem}>
              <Text style={styles.bmiValue}>
                {parsedWeight ? `${parsedWeight.toFixed(1)} kg` : "--"}
              </Text>
              <Text style={styles.bmiLabel}>Peso</Text>
            </View>
            <View style={styles.bmiItem}>
              <Text style={styles.bmiValue}>
                {heightMeters > 0 ? `${heightMeters.toFixed(2)} m` : "--"}
              </Text>
            ...
            <View style={[styles.bmiItem, styles.bmiStatusItem]}>
              ...
              <Text style={[styles.bmiStatusValue, { color: bmiStatusColor }]}>
                {bmiStatusLabel}
              </Text>
            </View>
          </View>
          ...
          >
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                styles.routeButton,
                {
                  backgroundColor: tracking ? "#FF5757" : "#A6FF00",
        ...
        <View style={[styles.cardLarge, { padding: 12 }]}>
          ...
          {routePoints.length === 0 && !lastSavedSession ? (
            <Text style={[styles.rowSub, { marginTop: 8 }]}>
              Activa <Text style={styles.italicText}>Iniciar ruta</Text> para
              comenzar a registrar tu recorrido.
            </Text>
          ) : null}
        ...
        <View style={styles.switcher}>
          <Ionicons
            ...
            size={20}
            color="#FFFFFF"
            onPress={() => setDayOffset((n) => Math.min(6, n + 1))}
          />
          ...
          <Ionicons
            ...
            size={20}
            color="#FFFFFF"
            onPress={() => setDayOffset((n) => Math.max(0, n - 1))}
          />
        </View>
          ...
          <View style={styles.modalSheet}>
            ...
            >
              ...
              <View style={[styles.bmiEditorRow, { marginBottom: 0 }]}>
                ...
                <TextInput
                  value={bmiForm.sex}
                  onChangeText={(text) =>
                    handleBmiChange("sex")(text.slice(0, 1).toUpperCase())
                  }
                  placeholder="M/F"
                  placeholderTextColor="#6B6B70"
                  maxLength={1}
            ...
            >
              {savingBmi ? (
                ...
                <Text style={styles.bmiSaveText}>Guardar</Text>
              )}
            </Pressable>
          </View>
        </View>
